...
class MainFrame(ContainerFrame):
    """
    MainFrame class serves as the main interface for the application, managing different modes and user interactions.

    Attributes:
        - Inherits from a superclass (not specified in the provided context).

    Methods:
        - start_mode: Sets the mode of the application and updates the ScaleFrame accordingly.
        - no_profile: Displays a window prompting the user to create a new profile if no active profile is found.
        - help_cmd: Displays the main help frame.
    """
...
Intra-Rater-Reliability-Untersuchung zu erstellen:"""
    ...
    def start_mode(self, mode):
        """
        Sets the mode of the application and updates the ScaleFrame accordingly.

        Args:
            mode (str): The mode to set for the application. Possible values are "analyse" or "rate".
        """
    ...
    def no_profile(self):
        """
        Displays a window prompting the user to create a new profile if no active profile is found.

        This method is called when there is no active profile in the application. It opens a new window
        where the user can input a name to create a new profile. If the user does not provide a name,
        an error message is shown. Once a valid name is provided, the profile is created and the window
        is closed.
        """
